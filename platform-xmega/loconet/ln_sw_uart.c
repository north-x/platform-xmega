...
#define LN_SW_UART_SET_TX_HIGH2 PORTD.OUTCLR = (1<<7);
#define LN_SW_UART_SET_TX_LOW2 PORTD.OUTSET = (1<<7);
...
ISR(TCE0_CCD_vect)
{
	...
	if( lnState == LN_ST_TX )
	{
		...
		// ARE NOT THE SAME as our circuit requires the TX signal to be INVERTED
		// If they are THE SAME then we have a Collision
		if( ( ( LN_TX_PORT2 >> LN_TX_BIT2 ) & 0x01 ) == ( ( filter_cnt ) & 0x01 ) )
		{
			lnBitCount = 0 ;
			lnState = LN_ST_TX_COLLISION ;
		}
...
void initLocoNetHardware( LnBuf *RxBuffer )
{
	...
	if ((USARTD1.CTRLA&USART_RXEN_bm) || (USB.CTRLA&USB_ENABLE_bm))
	{
		// PORTD 6:RX 7:TX
